...
def avoid(sonar,lspeed,rspeed):
    ...
    if (sonar[2] < 0.2) or (sonar[3] < 0.2) or (sonar[1] < 0.2) or (sonar[0] < 0.1) :
        lspeed = 0.9
        rspeed = -0.9
        print('OBJETO A IZQ')
    
    elif (sonar[4] < 0.2) or (sonar[5] < 0.2) or (sonar[6] < 0.2) or (sonar[7] <0.1): #or (sonar[2] < 0.3):
        lspeed = -0.9
        rspeed = 0.9
        print('OBJETO A DCHA')
